...
class HudLaneWriterTest {
    ...
    @Test fun `feature ids follow the donor layout`() {
        assertEquals(427_827_416, HudLaneWriter.FID_LANE_COUNT)
        assertEquals(427_827_288, HudLaneWriter.fidGlyph(0))
        assertEquals(427_827_296, HudLaneWriter.fidState(0))
        assertEquals(427_827_300, HudLaneWriter.fidValid(0))
        // Slots are 16 apart.
        assertEquals(427_827_288 + 7 * 16, HudLaneWriter.fidGlyph(7))
        assertEquals(427_827_296 + 7 * 16, HudLaneWriter.fidState(7))
        assertEquals(427_827_300 + 7 * 16, HudLaneWriter.fidValid(7))
    }
    ...
    @Test fun `three lanes with the middle one recommended`() {
        ...
        assertEquals(HudLaneWriter.STATE_PLAIN, m[HudLaneWriter.fidState(0)])
        assertEquals(HudLaneWriter.VALID, m[HudLaneWriter.fidValid(0)])
        // Slot 1: recommended straight. Donor active glyph = dir + 26, straight dir = 0.
        assertEquals(26, m[HudLaneWriter.fidGlyph(1)])
        assertEquals(HudLaneWriter.STATE_RECOMMENDED, m[HudLaneWriter.fidState(1)])
        assertEquals(HudLaneWriter.VALID, m[HudLaneWriter.fidValid(1)])
        // Slot 2: plain right, code 3.
        assertEquals(NavLaneCodes.RIGHT + 1, m[HudLaneWriter.fidGlyph(2)])
        assertEquals(HudLaneWriter.STATE_PLAIN, m[HudLaneWriter.fidState(2)])
    }
    ...
    @Test fun `unused slots are switched off`() {
        ...
        for (slot in 1 until NavLanes.MAX_LANES) {
            ...
            assertEquals("slot $slot state", HudLaneWriter.STATE_UNUSED, m[HudLaneWriter.fidState(slot)])
            assertEquals("slot $slot valid", HudLaneWriter.INVALID, m[HudLaneWriter.fidValid(slot)])
        }
    }
    ...
    @Test fun `no lanes clears the strip and blanks every slot`() {
        ...
        for (slot in 0 until NavLanes.MAX_LANES) {
            assertEquals(HudLaneWriter.INVALID, m[HudLaneWriter.fidValid(slot)])
        }
    }
    ...
    @Test fun `a multi-direction lane uses the combined glyph for the direction taken`() {
        ...
        assertEquals(HudLaneWriter.STATE_RECOMMENDED, m[HudLaneWriter.fidState(0)])
    ...
    @Test fun `more lanes than slots are dropped, not wrapped`() {
        ...
        for (slot in 0 until NavLanes.MAX_LANES) {
            assertEquals(HudLaneWriter.VALID, m[HudLaneWriter.fidValid(slot)])
        }
    }
